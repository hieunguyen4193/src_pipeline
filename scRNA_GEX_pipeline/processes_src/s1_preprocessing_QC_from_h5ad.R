...
s1.input.raw.data <- function(path2input,
  ...
  all_exprs <- Sys.glob(file.path(path2input, "*.count_matrix.csv"))
  
  # assign folder names as name of the experiment data. 
  names(all_exprs) <- to_vec(for(exprs in all_exprs) str_replace(basename(exprs), ".count_matrix.csv", "")) 
  ...
  #####--------------------------------------------------------------------------#####
  ##### convert the h5ad file to count matrix and metadata file by this python scrpt
  # import scanpy as sc
  # import os
  # 
  ...
  # 
  # then just read the file with read.csv.
  #####--------------------------------------------------------------------------#####
  for (i in seq_along(all_exprs)){
    path_to_expr <- all_exprs[i]
    input.data <- read.csv(path_to_expr) 
    expr.name <- names(all_exprs)[i]
    input.data <- input.data %>% column_to_rownames("X") %>% as.matrix()
