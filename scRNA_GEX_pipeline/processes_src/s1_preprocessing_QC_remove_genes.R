...
s1.input.raw.data <- function(path2input,
  ...
  for (i in seq_along(all_exprs)){
    ...
    # remove genes based on the input
    if (is.null(remove.genes) == FALSE){
      keep.genes <- to_vec(for (item in row.names(input.data)) if (item %in% remove.genes == FALSE) item)
      input.data <- input.data[keep.genes, ]  
      print(sprintf("remove the genes %s out of the input data", paste(remove.genes, collapse = ", " )))
    }
