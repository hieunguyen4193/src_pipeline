...
run_pipeline_GEX <- function(path2src,
      ...
      if (rerun[["s1"]] == TRUE | file.exists(file.path(path.to.output, "s1_output", sprintf("%s.output.s1.rds", PROJECT)))  == FALSE){
        ...
        if (input.method == "normal"){
          ...
          write(sprintf("Filtered %s cells based on the result of 1st rounds", length(filtered.barcodes)), 
                file=log.file, 
                append=TRUE)
        } else if (input.method == "from_count_matrix") {
          source(file.path(path2src, "s1_preprocessing_QC_from_count_matrix.R"))
          s.obj <- s1.input.raw.data(path2input = path2input, 
                                     stage_lst = stage_lst, 
                                     MINCELLS = MINCELLS, 
                                     MINGENES = MINGENES,
                                     ...
                                     save.RDS.s1 = save.RDS[["s1"]],
                                     path.to.output = path.to.output)
          write("Using source file from s1_preprocessing_QC_from_count_matrix.R", 
                file=log.file, 
                append=TRUE)
          ...
          s.obj <- s1.input.raw.data(path2input = path2input, 
                                     ...
                                     path.to.count.clonaltype = path.to.count.clonaltype,
                                     filtered.barcodes = filtered.barcodes)
        } else if (input.method == "from_rds"){
          source(file.path(path2src, "s1_preprocessing_QC_from_rds.R"))
          s.obj <- s1.input.raw.data(path2input = path2input, 
                                     stage_lst = stage_lst, 
                                     MINCELLS = MINCELLS, 
                                     MINGENES = MINGENES,
  ...
  if (sw[["s8"]] == "on"){
    ...
    if (rerun[["s8"]] == TRUE | file.exists(file.path(path.to.output, "s8_output", sprintf("%s.output.s8.rds", PROJECT)))  == FALSE){
      ...
      ##### UPDATE 18.04.2024: SEURAT V5 INTEGRATION WORKFLOW
      s.obj <- s8.integration.and.clustering_V5(s.obj,
                                                save.RDS[["s8"]],
                                                path.to.output,
                                                use.sctransform,
                                                num.PCA,
