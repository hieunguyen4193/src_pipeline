...
s1.input.raw.data <- function(path2input,
  ...
  if (is.null(filtered.barcodes) == FALSE){
    ...
    cells_to_keep <- subset(all_cells, all_cells %in% filtered.barcodes == FALSE)
  ...
  ##############################################################################
  # __________ FILTER CELLS THAT DO NOT HAVE CLONAL TYPE INFORMATION __________
